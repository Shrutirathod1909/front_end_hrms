...
const BranchPage = () => {
  ...
  const fetchCompanies = async () => {
    try {
      const res = await axios.get(`${API_URL}/api/companies`, getAuthHeaders());
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = isEditing
        ? `${API_URL}/api/branches/${isEditing.id}`
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this branch?")) {
      try {
        await axios.delete(`${API_URL}/api/branches/${id}`, getAuthHeaders());
  ...
  const handleDownloadExcel = async () => {
    try {
      ...
      const response = await axios.get(`${API_URL}/api/branches/export`, {
        params,
